import { Text, View, StyleSheet, TouchableOpacity, Image, ScrollView, PermissionsAndroid, YellowBox, Modal, StatusBar } from 'react-native';
...
export class HomeScreen extends Component {
  ...
  addpost() {
    ...
    this.setState({ posts: posts_arr, modal_visible: false, photo_uri: '', details: "", error_text: "" })
  }
  renderposts() {
      ...
      <View style={styles.main_view_style}>
        <View style={styles.header}>
          ...
          <View style={{ width: '18%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginRight: RFValue(MARGIN.xsMargin) }}>
            <TouchableOpacity onPress={() => { this.setState({ modal_visible: true }) }} >
          ...
          <View style={styles.moddal_container_style}>
            <ScrollView showsVerticalScrollIndicator ={false}>
              ...
              <View style={styles.buttonView}>
                <GeneralButton
                  onPress={() => { this.state.photo_uri != "" ? this.addpost() : this.warn_msg() }}
                  title="نشر"
                  bgcolor={COLORS.primary}
                  activeOpacity={0.7} />
                <Text style={{ color: COLORS.error }}>{this.state.error_text}</Text>
              </View>
            </ScrollView>
          </View>
