...
function PhotographerSettingScreen({navigation}) {
  ...
  const [phonenumersaddmodelvisible, setphonenumbersaddmodelvisible] =
    useState(false);
  ...
  const [
    additional_address_modal_visible,
    setadditional_address_modal_visible,
  ] = useState(false);
  ...
  const [address_under_bottom_error, setaddress_under_bottom_error] =
    useState('');
  //map
  ...
  function upload_img(photo_data) {
    RNFetchBlob.fetch(
      'POST',
      'https://generation3.000webhostapp.com/project/Training/upload.php',
      {
  ...
  async function _findMe() {
    watchID = await navigator.geolocation.watchPosition(({coords}) => {
  ...
  const delete_address = addressid => {
    ...
    axios
      .post(
        'https://generation3.000webhostapp.com/project/Training/delete_address.php',
        data_to_send,
      )
      .then(res => {
        if (res.status == 200) {
  ...
  const send_photographer_data = () => {
    ...
    axios
      .post(
        'https://generation3.000webhostapp.com/project/Training/insert_brand_details.php',
        data_to_send,
      )
      .then(res => {
        if (res.status == 200) {
    ...
    return phonenumbersarr.map((item, index) => {
      return (
        <View style={styles.each_additinal_phone_number_view_style}>
          <TouchableOpacity
            style={{width: '10%', padding: RFValue(1)}}
          ...
          <View style={{width: '90%'}}>
            <Text style={{color: COLORS.black, fontSize: RFValue(FONTS.h5)}}>
              {item}
            </Text>
          </View>
        </View>
      ...
      return (
        <View style={styles.each_additinal_phone_number_view_style}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              deleteaddress(index);
              delete_address(item.address_id);
            }}>
            <Feather
              name="x"
              size={RFValue(ICONS.lIcon)}
  ...
  const savechangespress = () => {
    ...
    if (phonenumbersarr.length == 0) {
      setphone_under_bottom_error(
        phone_under_bottom_error => 'يجب ادخال رقم الهاتف',
      );
    } else {
      setphone_under_bottom_error(phone_under_bottom_error => '');
    }
    if (additionaladdressesarr.length == 0) {
      setaddress_under_bottom_error(
        address_under_bottom_error => 'يجب ادخال العنوان',
      );
    } else {
    ...
    if (
      name != '' &&
      brandname != '' &&
      phonenumbersarr.length != 0 &&
      additionaladdressesarr.length != 0
    ) {
  ...
  const confirm_add_address = () => {
    ...
    if (additinal_address_value == '') {
      setadditional_address_error(
        additional_address_error => 'يجب ادخال تفاصيل العنوان',
      );
    }
    if (location.latitude == 0 && location.longitude == 0) {
      alert('يجب تحديد العنوان علي الخريطه');
    } else if (
      additinal_address_value != '' &&
      location.latitude != 0 &&
      location.longitude != 0
    ) {
      additional_addresses_arr.push(obj);
      setadditionaladdressesarr(
        additionaladdressesarr => additional_addresses_arr,
      );
      setadditinal_address_value(additinal_address_value => '');
      setadditional_address_error(additional_address_error => '');

      setadditional_address_modal_visible(
        additional_address_modal_visible => false,
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        ...
        {isLoading ? (
          <ActivityIndicator size={RFValue(40)} color={COLORS.primary} />
        ) : (
          <View>
            ...
            <View>{renderphonenumbers()}</View>
            <View style={styles.buttonView}>
              <GeneralButton
                onPress={() =>
                  setphonenumbersaddmodelvisible(
                    phonenumersaddmodelvisible => true,
                  )
                }
                ...
                activeOpacity={0.7}
              />
              <Text style={styles.error_text_style}>
                {phone_under_bottom_error}
              </Text>
            </View>

            <View>{render_additional_addresses()}</View>
            <View style={styles.buttonView}>
              <GeneralButton
                onPress={() => {
                  setadditional_address_modal_visible(
                    additional_address_modal_visible => true,
                  );
        ...
        <View style={styles.addphonemodalcontainerstyle}>
          <View style={[styles.headerView, {marginBottom: RFValue(60)}]}>
            <TouchableOpacity
              onPress={() =>
                setphonenumbersaddmodelvisible(
                  phonenumersaddmodelvisible => false,
                )
              }>
              <AntDesign
                name="arrowright"
        ...
        <View style={{padding: RFValue(PADDING.xsPadding)}}>
          ...
          <View>
            <Input
              ...
              onChangeText={value =>
                setadditinal_address_value(additinal_address_value => value)
              }
            />
            <Text style={styles.error_text_style}>
              {additional_address_error}
            </Text>
          </View>
          <View style={styles.buttonView}>
            <GeneralButton
